...
export default function Login() {
  ...
  if (isMobile) return (
    <div style={{
      ...
      {/* BG blobs */}
      <div style={{ position:'absolute', top:'-15%', left:'-20%', width:280, height:280, borderRadius:'50%', background:'rgba(99,102,241,0.18)', pointerEvents:'none' }} />
      <div style={{ position:'absolute', bottom:'-10%', right:'-15%', width:220, height:220, borderRadius:'50%', background:'rgba(139,92,246,0.12)', pointerEvents:'none' }} />

      {/* Logo */}
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:'2.5rem', position:'relative', zIndex:1 }}>
        <div style={{ width:44, height:44, borderRadius:12, background:'linear-gradient(135deg,#6366f1,#8b5cf6)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.4rem', flexShrink:0 }}>📦</div>
        <div>
          <div style={{ color:'#fff', fontWeight:900, fontSize:'1.2rem', letterSpacing:'-0.5px', lineHeight:1 }}>
            SES<span style={{ color:'#6366f1' }}>DIAN</span>
          </div>
          <div style={{ color:'#475569', fontSize:'0.55rem', fontFamily:'JetBrains Mono', letterSpacing:2 }}>ASSET MANAGEMENT</div>
        </div>
      </div>

      {/* Heading */}
      <div style={{ position:'relative', zIndex:1, marginBottom:'2rem' }}>
        <h1 style={{ color:'#fff', fontSize:'2rem', fontWeight:800, lineHeight:1.2, marginBottom:'0.75rem' }}>
      ...
      <div style={{
        ...
        <h2 style={{ color:'#fff', fontWeight:800, fontSize:'1.3rem', marginBottom:'0.35rem' }}>Masuk</h2>
        <p style={{ color:'#64748b', fontSize:'0.8rem', marginBottom:'1.25rem' }}>Gunakan NIP dan password Anda</p>

        {reason==='inactivity' && <div style={alertYellow}>⚠️ Sesi berakhir karena tidak aktif.</div>}
        ...
        <div style={{ marginBottom:'1rem' }}>
          ...
          <input value={form.nip} onChange={e => setForm({...form, nip:e.target.value})}
            placeholder="18 digit NIP Anda" inputMode="numeric"
            style={inpDark}
        ...
        <div style={{ marginBottom:'1.25rem' }}>
          ...
          <div style={{ position:'relative' }}>
            <input type={showPass?'text':'password'} value={form.password}
              ...
              placeholder="Password" style={{ ...inpDark, paddingRight:'3rem' }}
              onFocus={e => e.target.style.borderColor='#6366f1'}
              onBlur={e  => e.target.style.borderColor='rgba(255,255,255,0.1)'} />
      ...
      <div style={{ width:480, display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem', position:'relative', zIndex:1 }}>
        <div className="animate-fade-up delay-2" style={{
          ...
          <div style={{ marginBottom:'1rem' }}>
            <label style={lbWhite}>NIP</label>
            <input value={form.nip} onChange={e => setForm({...form, nip:e.target.value})}
              placeholder="18 digit NIP Anda" style={inpDark}
              onFocus={e => e.target.style.borderColor='#6366f1'}
              onBlur={e  => e.target.style.borderColor='rgba(255,255,255,0.1)'} />
          </div>
...
const lbWhite    = { fontSize:'0.75rem', fontWeight:700, color:'#94a3b8', display:'block', marginBottom:6, textTransform:'uppercase', letterSpacing:0.5 }
